...
void HariMain(void){
	...
	fifo32_init(&fifo,128, fifobuf);	//初始化键盘缓冲
	...
	sprintf(s, "memeory %dMB  free: %dKB", 
	...
	// 定义tss_b的值
	tss_b.eip = (int) &task_b_main; // 定义切换到该任务时，从哪里开始运行
	tss_b.eflags =  0x00000202; // STI后的EFLAGS值就是0x00000202
	tss_b.eax = 0;
	tss_b.ecx = 0;
	tss_b.edx = 0;
	tss_b.ebx = 0;
	tss_b.esp = task_b_esp; // task_b_esp 任务B的任务栈
	tss_b.ebp = 0;
	tss_b.esi = 0;
	tss_b.edi = 0;
	tss_b.es  = 1 * 8;
...
void make_window8(unsigned char* buf, int xsize, int ysize, char* title){
	...
	boxfill8(buf, xsize, COL8_000084, 0				 , ysize - 1,	xsize - 1, ysize - 1);
