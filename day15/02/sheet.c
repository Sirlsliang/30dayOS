...
void sheet_updown(struct SHEET* sht, int height){
	...
	if (old > height){	// 比以前低
		...
		sheet_refreshsub(ctl, sht->vx0, sht->vy0, 
											sht->vx0 + sht->bxsize, sht->vy0 + sht->bysize, height, height); // 按新图层信息重新绘制画面
	}
	return;
}
...
void sheet_free(struct SHEET* sht){
	if (sht->height >= 0){
		sheet_updown(sht, -1); // 如果处于显示状态，先设定为隐藏
	}
	sht->flags = 0; // 设定未使用标志
	return;
}
...
void sheet_refreshsub(struct SHTCTL* ctl, int vx0, int vy0,
	...
	for (h = h0; h <= h1; h++){
		...
		sid = sht - ctl->sheets0;
		// 利用vx0 - vy1，对bx0-by1倒推
		bx0 = vx0 - sht->vx0;
		by0 = vy0 - sht->vy0; 
		bx1 = vx1 - sht->vx0;
		by1 = vy1 - sht->vy0; //  判断刷新处与图层的交集
